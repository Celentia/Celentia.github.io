...
// page - текущая (открытая - номер) страница, то есть в ф-ю передаем номер текущий страницы, контент котор впоследствии выводим
    this.showPage = function(page) {
        ...
        for (var i = (page-1) * this.paragraphsPerPage; i < ((page-1)*this.paragraphsPerPage) + this.paragraphsPerPage; i++) {
            ...
            html += '<div class="img-wrapper" onclick="FunctionName(this)">' + $(this.paragraphsInner).eq(i).outerHTML() + '</div>';
            //console.log($(this.paragraphs).eq(i).html());
            console.log($(this.paragraphsInner).eq(i).outerHTML());
        }
...
//}
    ...
    pager.paragraphsInner = $('img', '.img-wrapper'); 
    pager.showPage(1);
});
